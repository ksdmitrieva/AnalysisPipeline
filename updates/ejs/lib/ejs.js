...
//added for Analysis
exports.createModel = function (template, opts) {
  var templ = new Template(template, opts);
  var parsed_arr = templ.parseTemplateText();
  var res_array = [];
  for (var i=0; i < parsed_arr.length; i++){
    if (parsed_arr[i] == '<%-' && parsed_arr[i+1].trim().indexOf("include") !== 0) {
      res_array.push({index: i, value: parsed_arr[i+1].trim(), type: "raw"});
    }
  }
