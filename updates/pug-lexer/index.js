...
Lexer.prototype = {
  ...
  interpolation: function() {
    if (/^#\{/.test(this.input)) {
      ...
      var tok = this.tok('interpolated-code', match.src);
      tok.mustEscape = true;
      this.tokens.push(tok);
  ...
  /**
   * Non-interpolated tag.
   * Added for Analysis
   */

  noninterpolation: function() {
    //console.log("noninterpolation "+this.input);
    if (/^!\{/.test(this.input)) {
      var match = this.bracketExpression(1);
      this.consume(match.end + 1);
      var tok = this.tok('interpolated-code', match.src);
      tok.mustEscape = false;
      this.tokens.push(tok);
      this.incrementColumn(2); // '!{'
      this.assertExpression(match.src);
  ...
  each: function() {
    ...
    //to parse old Jade files, not just Pug files
    //in Jade -each and -for are treated as Scala code
  ...
  advance: function() {
    return this.callLexerFunction('blank')
      ...
      || this.callLexerFunction('interpolation')
            //added for Analysis
      || this.callLexerFunction('noninterpolation')
